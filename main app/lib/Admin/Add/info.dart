    ...
    return TextFieldContainer(
      child: TextFormField(
        ...
        validator: (value) => value.isEmpty
            ? 'Required'
            : (titleRegExp.hasMatch(value) ? null : 'Not a valid title'),
        onSaved: (value) {
    ...
    return TextFieldContainer(
      child: TextFormField(
        ...
        decoration: InputDecoration(
          prefixIcon: Padding(
            padding:
                const EdgeInsets.only(left: 0, top: 0, right: 0, bottom: 78),
            child: Icon(
        ...
        validator: (value) => value.isEmpty
            ? 'Required'
            : (titleRegExp.hasMatch(value) ? null : 'Not a valid description'),
        onSaved: (value) {
          // or _emailController.text = value!;
    ...
    return RoundedButton(
      ...
      press: () async {
        if (_formKey.currentState.validate()) {
          ...
          dynamic result = await _auth.addInformation(
              _authData['title'].trim(), _authData['description'].trim());

          if (result == "success info") {
            ourViewMode.showTopSnackBar(context, 'Success',
                'Information has been posted', Icons.add_task); // change
          ...
          } else // error
          {
            ourViewMode.showTopSnackBar(
              context,
              'Couldn\'t post',
              'An error occurred while posting info',
