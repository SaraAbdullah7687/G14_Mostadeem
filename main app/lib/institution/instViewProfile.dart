...
class InstViewProfile extends StatelessWidget {
  final ViewInstitutionViewModel ourViewMode =
      ViewInstitutionViewModel(); // use the other one in admin class
  //String UID = 'CXbJCQp7PhZRveFPvp6G6j0uG4o2'; // يجي من افنان
  AuthService auth = AuthService();
  DatabaseService db = DatabaseService();
  String UID;
  InstViewProfile({this.UID});
...
@override
            ...
            builder: (context, snapshot) {
              ...
              return Stack(
                children: [
                  ...
                  Container(
                    margin: const EdgeInsets.only(top: 30, left: 9),
                    child: IconButton(
                      icon: const Icon(
  ...
  Widget bottomWidgetContent(BuildContext context, dynamic userDocument) {
    // bring from DB using inst UID
    return Column(
      // do we add button for initiate req??
      children: [
// Name
        Padding(
...
// Social media
  ...
  Widget instName(dynamic userDocument) {
    return Text(
      userDocument['name'],
      style: TextStyle(
          color: kPrimaryColor, fontSize: 35.0, fontWeight: FontWeight.bold),
    );
  }
  ...
  Widget instRating(dynamic userDocument) {
    return Row(
      ...
      children: <Widget>[
        RatingBarIndicator(
          itemSize: 30,
          rating: double.parse(userDocument['ratesAvg']),
          ...
          Flexible(
            child: Container(
              ...
              child: SocalIcon(
                ...
                color: kPrimaryColor,
                press: () =>
                    ourViewMode.goToTwitter(userDocument["twitterAccount"]),
              ),
            ),
          ),
          SizedBox(width: 40),
